...
define(function(require,exports,module) {
    ...
    var model = new Vue({
        ...
        methods:{
            ...
            allPay : function () {//合并付款
                for(var i=0;i<this.multipleSelection.length;i++){
                    if(this.multipleSelection[i].status=='W'){
                        model.idsArr.push(this.multipleSelection[i].id);
                        model.payFlag = true;
                    }else{
                        model.payFlag = false;
                    }
                }
                if(model.idsArr.length==0){
                    var alert = Vue.$alert({
                        visible:true,
                        msgType:error,
                        text1:'请选择至少一条代付款交易',
                    ...
                    location.href = 'trans_payment.html?tranIds='+model.idsArr.join(',')+'&tranNo='+model.tranNo;
                }
