    ...
    var model = new Vue({
        ...
        beforeCreate : function () {
            ...
            this.$http({
                ...
                function (res) {
                    self.creditScore = res.data.data.iou.creditScore || 0;
                    self.creditScoreLastMonth = res.data.data.iou.lastCreditScore || 0;
                    var evalDate = res.data.data.iou.creditAssessTime.toString().trim();
                    if(evalDate) {
                        if(evalDate.split(' ')[0]) {
                            self.evalDate = evalDate.split(' ')[0];
                        } else {
